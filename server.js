...
app.post('/api/send-telegram', authenticateToken, async (req, res) => {
    ...
    // إن كان النص يحتوي بالفعل على الهيدر المنسق من الواجهة يتم إرساله مباشرة، وإلا يتم تنسيقه
    const formattedMsg = message.includes('جامعة البصرة')
        ? `${message}\n\n<i>this website powered by <b>Ph.Mohammed ibrahim</b></i>`
        : `📢 <b>جامعة البصرة - كلية الصيدلة</b>\n\n${message}\n\n<i>this website powered by <b>Ph.Mohammed ibrahim</b></i>`;

    const success = await sendTelegramNotification(formattedMsg);

    if (success) {
        res.json({ success: true, message: 'تم نشر الإشعار على القناة بنجاح!' });
    } else {
        res.status(500).json({ success: false, error: 'فشل إرسال الإشعار، تأكد من إعدادات البوت والقناة.' });
    }
});
